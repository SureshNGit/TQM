...
namespace TQM
{
    ...
    public partial class DataSync : ContentPage
    {
        ...
        private async void toggleLoading(bool visibility)
        {
            Device.BeginInvokeOnMainThread(() =>
            {
                ...
                if (visibility == false)
                {
                    ...
                    backupButton.BackgroundColor = Color.FromHex("#0e0273");
                }
            });
        }

        private async void toggleBackUpButton(bool visibility)
        {
            Device.BeginInvokeOnMainThread(() =>
            {
                backupButton.IsEnabled = visibility;
                if (visibility)
                {
                    backupButton.BackgroundColor = Color.FromHex("#0e0273");
                }
                ...
                {
                    backupButton.BackgroundColor = Color.SlateGray;
                }
            });
        }
        ...
        private async void backupButton_Clicked(object sender, EventArgs e)
        {
            ...
            await Task.Run(async () => await syncDataCompanyModel(), ct);
            src.Cancel();
        }
