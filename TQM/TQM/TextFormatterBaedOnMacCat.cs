...
namespace TQM
{
    public class TextFormatterBaedOnMacCat : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            var parm = (decimal)values[1];

            if (machineCat != "Spinning" && machineCat != "Winding")
            {
                return formatDecimal(parm, 4).ToString();
            }

            if (machineCat == "Spinning" || machineCat == "Winding")
            {
                return formatDecimal(parm, 2).ToString();
            }
